...
Example 1: Input: dungeon = [[-2,-3,3],[-5,-10,1],[10,30,-5]] Output: 7
Explanation: The initial health of the knight must be at least 7 if he follows the optimal path: RIGHT-> RIGHT -> DOWN -> DOWN.
Example 2: Input: dungeon = [[0]] Output: 1
...
var calculateMinimumHP = function (dungeon) {
  ...
  const minHP = {};
  const priorityQueue = new MinPriorityQueue({ compare: (c1, c2) => c2.minHP - c1.minHP });
  let finalHP = -Infinity;
  priorityQueue.enqueue({ y: 0, x: 0, hp: dungeon[0][0], minHP: dungeon[0][0] });
  while (!priorityQueue.isEmpty()) {
    //Get best HP (the highest one)
    const node = priorityQueue.dequeue();
    // console.log(`Checking node [${node.y},${node.x}] = ${node.hp} min=${node.minHP}`);
    if (node.x == width - 1 && node.y == height - 1) {
      // Goal reached
      finalHP = Math.max(node.minHP, finalHP); // Get the 
      // console.log(`Reached end with HP ${node.hp} min=${node.minHP}, finalHP=${finalHP}`);
    } else {
      checkCell(node.y, node.x + 1, node); //Check right
      checkCell(node.y + 1, node.x, node); // Check down
    }
  }
  return Math.max(-finalHP + 1, 1);

  function checkCell(checkY, checkX, node) {
    if (checkY >= height || checkX >= width) {
      return;
    }
    const hp = node.hp + dungeon[checkY][checkX];
    const minHP = Math.min(node.minHP, hp);
    priorityQueue.enqueue({ y: checkY, x: checkX, hp: hp, minHP: minHP });
    // console.log(`Adding node [${checkY},${checkX}] = ${hp}`);
  }
};
...
// Does not work with [[1,2,1],[-2,-3,-3],[3,2,-2]]
var calculateMinimumHP = function(dungeon) {
  ...
  const minHP = {};
  const maxHP = {};
  minHP[`${height},${width-1}`] = 1; // Set one value for below the end cell.
  maxHP[`${height},${width-1}`] = 1;
  for (let y = height - 1; y >= 0; y--) {
    for (let x = width - 1; x >= 0; x--) {
      const downKey = `${y+1},${x}`;
      const rightKey = `${y},${x+1}`;
      const maxDown = maxHP[downKey] || Infinity ;
      const maxRight = maxHP[rightKey] || Infinity;
      let key;
      if (maxRight == maxDown) {
        const minDown = minHP[downKey] || -Infinity ;
        const minRight = minHP[rightKey] || -Infinity;
        key = minRight <= minDown ? rightKey : downKey;
      } else {
        key = maxRight < maxDown ? rightKey : downKey;
      }
      let min = minHP[key] - dungeon[y][x];
      // Choose right cell
      if (min <= 0) {
        min = Math.max(1, min);
      }
      const max = Math.max(min, maxHP[key]);          
      console.log(`[${y},${x}]: ${min}/${max} from ${key} maxDown=${maxDown},maxRight=${maxRight}`);
      minHP[`${y},${x}`] = min;
      maxHP[`${y},${x}`] = max;
    }
  }
